...
def playgame(questions, highscores):
    print()
    amount_correct=0
    ...
    for num in range(0, 10):
        ...
        if num <= 8:
            print(
                ...
                (amount_correct, num + 1))
        elif num == 9:
            print(
                'Your final score is %d out of %d' %
                (amount_correct, num + 1))
    highscore=amount_correct / num
    if highscore >= 0:
        print('Congratulations! You received a high score!')
        username=input('Please enter a username: ')
        print()
        highscores[username]=highscore
    again=input('Want to play again? ').lower()
    if again == 'yes':
        playgame(questions, highscores)
    else:
...
def viewhighscores(highscores):
    print()
    if highscores == {}:
        print('There are no highscores to display.')
    else:
        score=sorted(highscores.items(), key=itemgetter(1))
        score.reverse()
        for index in score:
            username=index[0]
            scored=round(index[1] * 100, 2)
            print(username + ': ' + str(scored) + '%')
    print()
    main()
